...
import { api, UpcomingClass } from '../../services/api';
...
const ScheduleScreen = () => {
  ...
  const loadSchedule = async () => {
    ...
    try {
      // Load schedule using dedicated /me endpoint
      const response = await api.getMySchedule(7);
      if (!response.success) {
      ...
      console.log('📅 Schedule data received:', scheduleData.length, 'items');
  ...
  const getClassesForSelectedDay = () => {
    let classes = upcomingClasses.filter(classItem => {
  ...
  const hasClassesOnDay = (day: any) => {
    return upcomingClasses.some(classItem => {
      // Parse date as local date to avoid timezone issues
      if (!classItem.date) return false;
      const [year, month, day_num] = classItem.date.split('-').map(Number);
      const classDate = new Date(year, month - 1, day_num);
      ...
      <View style={{ flex: 1 }}>
        ...
        <View style={commonStyles.header}>
          <View style={styles.logoContainer}>
            ...
            <View>
              <Text style={styles.universityName}>My Timetable</Text>
            </View>
          </View>
        ...
        <Card style={styles.classesCard}>
          ...
          {classesForDay.length === 0 ? (
            ...
            classesForDay.map((classItem, index) => (
              <View key={`${classItem.section_id}-${classItem.date}-${classItem.start_time}`} style={commonStyles.listItemWithLine}>
                ...
                <View style={commonStyles.itemContent}>
                  ...
                  <View style={styles.classInfo}>
                    ...
                    <View style={styles.classInfoRow}>
                      ...
                      <Text style={styles.classInfoText}>
                        {classItem.room_name || 'Room TBA'} • {classItem.building_name || 'Building TBA'}
                      </Text>
                    </View>
                    <View style={styles.classInfoRow}>
